                  ...
                  <div>
                    <div class="d-flex justify-content-between align-items-center" id="user-name">
                      <p class="mb-1">
                        {{-- @php
                        dd(getNameuser(1))
                        $nameuser = getNameuser($comment['userId']);
                        foreach ($nameuser as $value) {
                          dd($value);
                        }
                        dd($comment->user->hoten)
                          $nameuser = getNameuser($comment['userId']);
                          echo $nameuser;
                          $nameuser = App\Models\Member::find($comment['userId'])->first('hoten');
                        @endphp --}}
                        {{$comment->user->hoten}}<span> - {{$comment->created_at->diffForHumans()}}</span>
                      </p>
                    </div>
                    <p class="mb-0">
                      {{$comment['body']}}
                        ...
                        @csrf
                          ...
                          <div class="form-group d-flex flex-start w-100">
                            <img class="rounded-circle shadow-1-strong me-3 mr-1"
                                src="{{Auth::guard('webuser')->user()->image_path}}" alt="{{Auth::guard('webuser')->user()->image_name}}" width="40" height="40"/>
