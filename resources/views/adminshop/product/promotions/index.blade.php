...
<!-- Content Wrapper. Contains page content -->
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          {{-- <form id="feature-form"> --}}
                            <input type="hidden" id="active-input-{{$item['id']}}" name="active-input" value="{{ $item->active }}">
                            {{-- <button type="button" id="toggle-feature-btn" name="button_active">
                                {{ $item->active ? 'Turn On' : 'Turn Off' }}
                            </button> --}}
                            <input type="button"
                              id="active-button-{{$item['id']}}"
                              data-id="{{$item['id']}}"
                        ...
                        <td>
                          <div class="btn-group">
                            <abbr title="Sửa">
                              <a href="{{ route('adminshop.promotion.edit', $item['id'])}}" class="mr-1">
                            ...
                            <abbr title="Xóa">
                              <a onclick="return confirm('Bạn có muốn xóa chương trình khuyến mãi này không ?')" href="{{ route('adminshop.promotion.delete', $item['id'])}}">
  ...
  <!-- /.content-wrapper -->
  <script>
    // document.getElementById("toggle-feature-btn").addEventListener("click", function() {
    //     var featureState = document.getElementById("feature-state");

    //     featureState.value = featureState.value === "1" ? "0" : "1";
    // });
    ...
    buttons.forEach(function(button) {
      ...
      // var activebutton = document.getElementById("active-button-" + id);
      // if(input.value === "1"){
