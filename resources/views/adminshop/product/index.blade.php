                  ...
                  @if(count($productList))
                    @foreach ($productList as $item)
                      <tr>
                        <td>{{$item['name']}}</td>
                        <td>
                          <img src="{{$item->image_path}}" alt="{{$item->image_name}}" width="150px" height="100px">
                        </td>
                        <td>{{$item['amount']}}</td>
                        <td>{{$item['price']}}</td>
                        ...
                        <td>
                          <div class="btn-group">
                            <abbr title="Sửa dữ liệu">
                              <a href="{{ route('adminshop.product.edit', $item['id'])}}" class="mr-1">
                            ...
                            <abbr title="Xóa dữ liệu">
                              <a onclick="return confirm('Bạn có muốn xóa sản phẩm này không ?')" href="{{ route('adminshop.product.delete', $item['id'])}}">
