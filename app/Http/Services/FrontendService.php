...
class FrontendService{
    public function index()
    {
        ...
        $discount_five_products = Product::whereHas('promotions')
            ->with('promotions')
            ->where('status', '=', 1)
            ->orderByDesc('id')
            ->take(5)
        ...
        $three_imported_products = Product::orderBy('id', 'desc')->where('id_trademark', '!=', $id_trademarks_vn)->where('status', '=', 1)->limit(3)->get();
        $five_imported_products = Product::orderBy('id', 'desc')->where('id_trademark', '!=', $id_trademarks_vn)->where('status', '=', 1)->skip(3)->take(5)->get();
        ...
        return view('user.page.home', compact('products', 'sliders', 'five_products_promotions', 'discount_five_products', 'five_products', 'three_imported_products', 'five_imported_products', 'nposts', 'posts'));
    }
}
